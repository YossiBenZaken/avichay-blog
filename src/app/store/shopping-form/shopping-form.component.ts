...
export class ShoppingFormComponent implements OnInit,OnDestroy {
  ...
  userId: string;
  constructor(private _router: Router,private _auth: AuthService,private _order: OrderService) { }

  ngOnInit(): void {
    this.userSubscription = this._auth.user$.subscribe(user => this.userId = user.uid);
  }
  ngOnDestroy():void {
